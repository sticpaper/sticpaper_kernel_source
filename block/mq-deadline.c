...
static int dd_init_sched(struct request_queue *q, struct elevator_type *e)
{
	...
	/* COQ-scheduler For non-rotate disk tune
	 * To reduce unnecessary scheduler delays */
	if (q->nr_hw_queues >= 1) {
		dd->front_merges = 0;
		dd->fifo_expire[DD_READ] = HZ / 250;
		dd->fifo_expire[DD_WRITE] = HZ / 125;
		dd->prio_aging_expire = HZ / 50;
		dd->writes_starved = 1;
	}
