...
static int mps_rec_snapshot(struct pte_record *mps_rec, uint32_t flags)
{
	...
	/**
	 * The Linux 5.10 Kernel lacks ("VMA_ITERATOR") and ("for_each_vma"),
	 * we need to replace them with rb-tree and VMA linked list changes.
	 * @Desc: Replace Linux 5.10 Kernel code with macros (LINUX_VERSION)
	 */
#ifdef CONFIG_MOON_KERNEL_NEW
	...
	/* zhaoyuenan: changes legacy vma traversal mode */
